...
def _get_ang_scale(ims, bgval, exponent='inf', constraints=None, reports=None):
    ...
    dfts = [fft.fftshift(abs(fft.fft2(im/255))) for im in ims]
    # imshow(dfts[0]*1000,dfts[0]*1000,dfts[0]*1000)
    # imshow(ims_apod[0], ims_apod[0], ims_apod[0])
...
def _similarity(im0, im1, numiter=1, order=3, constraints=None,
    ...
    * Calculates precision.
    ...
    for ii in range(numiter):
        ...
        print("angle, scale",newangle, newscale)
        scale = scale*newscale
        angle = angle+newangle

        constraints_dynamic["scale"][0] = constraints_dynamic["scale"][0]/newscale
        constraints_dynamic["angle"][0] = constraints_dynamic["angle"][0]-newangle
